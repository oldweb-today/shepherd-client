...
var AUDIO_TYPES = [{ "id": "mp3",
  "type": MP3_MIME_TYPE }, { "id": "opus",
  "type": OPUS_MIME_TYPE }];

exports.getBestAudioType = getBestAudioType;
exports.WSAudio = WSAudio;
...
function WSAudio(browser_info, init_params) {
  ...
  var ws_open = function ws_open(event) {
    //this.send("ready");
  };

  var ws_message = function ws_message(event) {
    if (this.errCount == 0) {
      this.queue(event.data);
    }
  };

  var ws_error = function ws_error() {
    this.audioError("WS Error");
  };

  var ws_close = function ws_close() {
