...
require('unfetch/polyfill/index');
...
var _audioMediasource = require('./audio-mediasource');

var _audioWebrtc = require('./audio-webrtc');
...
function CBrowser(reqid, target_div, init_params) {
  ...
  function setup_browser() {
    ...
    var width = Math.max(w, min_width);
    var height = Math.max(h, min_height);
    ...
    if (init_params.webrtc) {
      ...
      audioType = (0, _audioMediasource.getBestAudioType)();
    }
  ...
  function handle_browser_response(data) {
    ...
    if (data.containers && data.containers.xserver && data.containers.xserver.ports) {
      var ports = data.containers.xserver.ports;
      ...
      if (init_params.audio && !window.audioPlugin) {
        if (audioType) {
          ...
          if (init_params.webrtc) {
            ...
            window.audioPlugin = new _audioWebrtc.WebRTCAudio("1", webrtc_data);
          } else {
            console.log("WSAudio: init plugin");
            window.audioPlugin = new _audioMediasource.WSAudio({ "audio": audioType,
