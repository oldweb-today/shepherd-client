...
function CBrowser(reqid, target_div, init_params) {
  ...
  function setup_browser() {
    ...
    req_params["audio"] = (0, _audioMediasource.getBestAudioType)();
  ...
  function handle_browser_response(data) {
    ...
    if (data.cmd_port && data.vnc_port) {
      ...
      if (init_params.audio) {
        ...
        if (data.audio) {
          if (window.location.href.indexOf("webrtc")) {
            // @todo CHANGE ME quick&dirty hack
            ...
            window.audioPlugin = new _audioMediasource.WSAudio(data, init_params);
          }
